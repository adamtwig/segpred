...
def getPrediction( args, groundTruthFile):
    ...
    if not args.predictionPath:
        ...
        #print(rootPath)
        #currFrame = args.predictionPath[-2:]
        #print(currFrame)
    '''
    #print(args.predictionPath)
...
#print(args.cityscapesPath)
trainTestVal = "val"
#trainTestVal = "train"
args.groundTruthSearch  = os.path.join( args.cityscapesPath , "gtFine" , trainTestVal, "*", "*_gtFine_labelIds.png" )
...
def evaluateImgLists(predictionImgList, groundTruthImgList, args):
    ...
    iterNum = args.predictionPath.split("_")[-1].strip('/')
    modelName = args.predictionPath.split("/")[-2]
    ...
    for label in args.evalLabels:
        labelName = id2label[label].name
        #if labelName in ['person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle', 'bicycle']:
        if True:
            classScoreList[labelName] = getIouScoreForLabel(label, confMatrix, args)
            if not np.isnan(classScoreList[labelName]):
                labelStr += labelName + " & "
                classOut = str(100.0*np.around(classScoreList[labelName],3))
                classStr += classOut + " & "
                verboseStr += labelName +": " + classOut + "\n" 
            #print(labelName, classScoreList[labelName])

    meanClassIOU = str(100.0*np.around(getScoreAverage(classScoreList, args),3))
    
    finalLabelStr = labelStr + "mean \\\ "
    finalClassStr = classStr + meanClassIOU + " \\\ \n"
    finalVerboseStr = verboseStr + "mean IoU: " + meanClassIOU + "\n"

    print(finalLabelStr)    
    print(finalClassStr)
    #outClassFile = 'joint_class_val.txt'
    #classFile = open(args.logFileName,'a')
    #classFile.write(finalVerboseStr)
    #classFile.write(finalClassStr)
    #classFile.close()
    ...
    if (not args.quiet):
        ...
        print("")
        '''
        #print("Mean Class IOU: {avg:5.3f}".format(avg=getScoreAverage(classScoreList, args)))
    ...
    for category in category2labels.keys():
        ...
        #print(category, categoryScoreList[category])
        if not np.isnan(categoryScoreList[category]):
            labelStr += category + " & "
            catStr += str(100.0*np.around(categoryScoreList[category],3)) + " & "
        #print(labelName, classScoreList[labelName])
    
    meanCatIOU = str(100.0*np.around(getScoreAverage(categoryScoreList, args),3))
    
    finalLabelStr = labelStr + "mean \\\ "
    finalCatStr = catStr + meanCatIOU + " \\\ \n"
    
    #outCatFile = 'joint_cat_val.txt'
    #catFile = open(outCatFile,'a')
    #catFile.write(finalCatStr)
    #catFile.close()    
    ...
    if (not args.quiet):
        ...
        print("")
        '''
        #print("Mean Category IOU: {avg:5.3f}".format(avg=getScoreAverage(categoryScoreList, args)))
...
# The main method
def main(model, dataset, results):
    global args
    
    args.model = model
    args.cityscapesPath = dataset
    args.predictionPath = results
    ...
    if True:
        ...
        #groundTruthImgList = "/working/results/cityscapes/ga_net/val/weakgt/"
